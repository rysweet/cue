...
suite('cueIntegration Test Suite', () => {
    ...
    test('Should check if cue is installed', async () => {
    ...
    test('Should handle cue bundled with extension', async () => {
        // cue is now bundled with the extension, so it should always be "installed"
        const extensionPath = path.join(__dirname, '..', '..', '..');
        const cuePath = path.join(extensionPath, 'bundled', 'cue', 'main.py');
        const fs = require('fs');
        
        // Check that the bundled cue exists
        const bundledExists = fs.existsSync(cuePath);
        assert.ok(bundledExists, 'Bundled cue should exist');
        
        // The checkcueInstalled method should return true for bundled cue
        const isInstalled = await cueIntegration.checkcueInstalled();
        assert.ok(isInstalled, 'cue should be detected as installed (bundled)');
    });
    ...
    test('Should build environment variables correctly', async () => {
        ...
        // Create new cueIntegration instance with mocked Neo4j manager
        const testcueIntegration = new cueIntegration(configManager, extensionPath, mockNeo4jManager);
        ...
        try {
            await testcueIntegration.analyzeWorkspace('/test/workspace', progress as any, token as any);
        } catch (error) {
