...
suite('Bundled Modules Test Suite', () => {
    ...
    test('cue module should exist', () => {
        ...
        assert.ok(fs.existsSync(cuePath), 'cue directory should exist');
        assert.ok(fs.existsSync(path.join(cuePath, 'main.py')), 'cue main.py should exist');
        assert.ok(fs.existsSync(path.join(cuePath, '__init__.py')), 'cue __init__.py should exist');
    });
